...
class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len,path):
        super().__init__()
        gene2vec_weight = np.load(path+'/gene2vec.npy')
...
class Gene_graph2VecEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len,path):
        super().__init__()
        gene2vec_weight = np.load(path+'/gene_interaction_graph.npy')
        gene2vec_weight = np.concatenate((gene2vec_weight, np.zeros((1, gene2vec_weight.shape[1]))), axis=0)
        gene2vec_weight = torch.from_numpy(gene2vec_weight)
        self.emb = nn.Embedding.from_pretrained(gene2vec_weight)
...
class Performer(nn.Module):
    def __init__(
        ...
        attn_route_map = {'mask': route_attn, 'pos_emb': route_attn,'cor_emb':route_attn}
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        if g2v_position_emb:
            self.pos_emb = Gene2VecPositionalEmbedding(dim, max_seq_len,path)
        ...
        if n2v_position_emb:
            self.cor_emb = Gene_graph2VecEmbedding(dim, max_seq_len,path)
            self.layer_cor_emb = Always(None)
        else:
            self.cor_emb = torch.zeros_like
            self.layer_cor_emb = Always(None)
    ...
    def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        ...
        layer_cor_emb = self.layer_cor_emb(x)##############
        
        if output_attentions:
            x, attn_weights = self.performer(x, pos_emb = layer_pos_emb, cor_emb=layer_cor_emb, output_attentions = output_attentions, **kwargs)#####################
            # norm and to logits
            x = self.norm(x)
            if return_encodings:
